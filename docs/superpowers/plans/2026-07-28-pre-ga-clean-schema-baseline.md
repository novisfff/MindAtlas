...
**Architecture:** A PostgreSQL-catalog canonicalizer produces stable object documents and SHA-256 fingerprints. Before the squash, it captures the exact `b6e2d4f8a901` schema, exact Legacy exclusion definitions, and every retained function/trigger definition. A deterministic generator renders a staged `pre_ga_v1_0001` root from live SQLAlchemy metadata plus that retained SQL-object registry. Disposable Database A (old chain) and Database B (staged root) must compare equal after only the committed exclusions and the separately validated schema-identity control table. One atomic archive activation moves all 60 old revisions outside Alembic's configured version location and makes the clean root the sole live revision. Fresh install is the default; a separate non-production-only rebaseline command may remove only verified, empty-of-business-state exclusion objects and stamp only after the resulting schema and retained data are proven unchanged. Runtime compatibility recomputes family, revision, structural fingerprint, seed/runtime contracts, deployment class, and code compatibility before API readiness or Worker claims.
...
- Consumes: all live `Base.metadata`, retained SQL-object registry, normalized pre-squash application document, and fixed family/identity constants.
...
def test_live_metadata_plus_retained_sql_matches_captured_clean_schema(
    ...
    actual = PostgresCatalogReader(empty_postgres_database.connection).read_document()
    actual = strip_schema_control_objects(actual)
    expected = load_pre_squash_snapshot().normalized_application_document
    assert_documents_equal(expected, actual)
```
...
5. model-reference fingerprint equals the normalized pre-squash fingerprint;
...
- Consumes: normalized application fingerprint, Plan 1 auth version, Plan 2 expected seed/runtime/codec/feature values, deployment class, and staged root.
...
The generated root includes literal normalized structural fingerprint, seed contract digest, runtime contract version, codec `3`, Capability feature digest, and Operator auth version. At upgrade time it validates deployment class, computes canonical runtime identity with the same compact sorted JSON algorithm, and inserts:
...
expected_payload = {
    ...
    "applicationStructuralFingerprint": normalized_source_fingerprint,
...
- Consumes: live old chain through `b6e2d4f8a901`, staged clean root, exclusion manifest, pre-squash snapshot, expected identity manifest, and canonical introspector.
...
def test_old_head_normalized_equals_clean_root(
    ...
    normalized_old = normalize_old_document(old_document, load_exclusion_manifest())
    normalized_clean = strip_schema_identity_control(clean_document)
    assert_documents_equal(normalized_old, normalized_clean)
```
...
- recomputed application fingerprint from B equals expected normalized fingerprint;
...
```python
left_bytes = canonical_json_bytes(normalized_old.to_payload())
right_bytes = canonical_json_bytes(normalized_clean.to_payload())
if left_bytes != right_bytes:
    raise SchemaComparisonError.from_documents(
        normalized_old,
        normalized_clean,
        safe_code="normalized_schema_difference",
    )
...
Expected: normalized canonical bytes and fingerprints are identical; Database B contains no exclusion object; all drift cases fail as expected; no PostgreSQL test skips.
...
The archive manifest binds all 60 old bytes to the pre-squash snapshot digest. `test_schema_archive.py` parses/hashes archive bytes but never imports or executes them. The one-time A/B proof from Task 7 remains represented by the committed identical canonical document/fingerprint and final evidence.
...
- committed old-normalized snapshot versus fresh root canonical equality;
...
- the exact old normalized canonical document equals a fresh root document byte-for-byte;
...
- ORM plus retained SQL registry differs from the normalized old-head document;
- generated root is not byte-reproducible or imports application/archive code;
- Database A/B canonical documents differ after only exact exclusions/control normalization;
...
- Canonical coverage check: namespaces, extensions, enums/domains, sequences, views/materialized views, columns/defaults/identity/generated state, PK/FK/unique/check/exclusion constraints, FK actions, indexes/expressions/predicates, complete functions, and complete trigger/linkage/enabled state all have explicit catalog sources and tests.
