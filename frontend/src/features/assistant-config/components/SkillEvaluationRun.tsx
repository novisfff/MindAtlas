...
import type { SkillTestTraceEvent, SkillTestRunStatus } from '../stores/skill-test-run-store'
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      {events.length === 0 ? (
        ...
        <ol className="max-h-80 space-y-2 overflow-auto rounded-md border p-3" aria-label={t('settings.universalSkills.evalTrace')}>
          {events.map((event) => (
