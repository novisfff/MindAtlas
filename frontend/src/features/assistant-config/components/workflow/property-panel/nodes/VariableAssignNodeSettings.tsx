  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <label className="text-sm font-semibold text-slate-700">{t('settings.skills.variableAssign.variable')}</label>
        <select
          value={variableName}
          onChange={(e) => onUpdate({ variableName: e.target.value })}
          className="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 bg-white hover:bg-slate-50 focus:ring-2 focus:ring-primary/20 focus:border-primary/50 outline-none transition-all appearance-none cursor-pointer"
          style={{
        ...
        {envOptions.length === 0 && (
          <div className="mt-2 text-xs text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-300 px-3 py-4 text-center">
      ...
      <div className="space-y-2">
        <label className="text-sm font-semibold text-slate-700">{t('settings.skills.variableAssign.operation')}</label>
        <select
          value={operation}
          onChange={(e) => onUpdate({ operation: e.target.value || 'set' })}
          disabled={!hasVariableSelected}
          className={`w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 outline-none transition-all appearance-none ${!hasVariableSelected ? 'bg-slate-50 cursor-not-allowed opacity-60' : 'bg-white hover:bg-slate-50 focus:ring-2 focus:ring-primary/20 focus:border-primary/50 cursor-pointer'}`}
          style={{
            backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`,
            backgroundPosition: 'right 0.75rem center',
            backgroundRepeat: 'no-repeat',
      ...
      {hasVariableSelected && operation !== 'clear' && (
        <div className="space-y-2">
          <label className="text-sm font-semibold text-slate-700">{t('settings.skills.variableAssign.valueTemplate')}</label>
          <div className="rounded-xl border border-slate-200 bg-white focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/50 transition-all overflow-hidden p-1 shadow-sm">
            <RichMentionInput
              value={valueTemplate}
              ...
              className="min-w-0 border-0 focus:ring-0 min-h-[56px]"
              multiline
            />
          </div>
        </div>
