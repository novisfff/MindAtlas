...
#### Scenario: OpenClaw workflow execution receives OpenClaw sys vars
- **WHEN** workflow 通过 OpenClaw capability runtime 执行且请求上下文带有 OpenClaw source/channel/session/tool 元数据
- **THEN** workflow SHALL 额外注入 `sys.openclaw_source`、`sys.openclaw_channel`、`sys.openclaw_session`、`sys.openclaw_tool`
